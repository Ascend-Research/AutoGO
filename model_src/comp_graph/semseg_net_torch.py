...
# This file is a modified change of model_src/comp_graph/tf_comp_graph_output_torch.py
# All it does differently is adjust a few parameters in the convolutions to make the network compatible with sseg
...
def get_op_model_for_node(node, op2i: OP2I, scope_name,
    ...
    if op_type == "conv":
        ...
        # Change here
        if node.resolution[1] < 20:
            stride = (1, 1)
            padding = "same"
            dil_rate = 2
        if node.resolution[1] < 10:
            dil_rate = 4
...
class CompGraphOutputNet(nn.Module):
    ...
    def __init__(self, op2i=None, cg=None,
        ...
        self.opi2io_shapes = {}
        op_categories = {}
        for ni, node in enumerate(topo_nodes):  # Topological sorting could be different
            self.opi2io_shapes[ni] = node.resolution
            scope_name = "op_{}/{}".format(ni, op2i.query_op(node.op_type_idx))  # Maybe change to have unique names per-op category
            op_type, op_model = get_op_model_for_node(node, op2i, scope_name,
                                                      op_override_dict=op_override_dict)
            if op_type in op_categories.keys():
                op_categories[op_type] += 1
            else:
                op_categories[op_type] = 0
            op_num = op_categories[op_type]
            scope_name = "op_{}{}/{}".format(op_type, op_num, op2i.query_op(node.op_type_idx))
    ...
    def forward(self, x):
        ...
        if len(self.output_node_inds) < 2:
            ...
            for oi in list(self.output_node_inds):
                out = self.lazy_padding(op_outputs[oi])
                final_out.append(out)
            final_out.sort(key = lambda x: x.shape[1], reverse=True)
        return final_out

    def lazy_padding(self, x):
        if x.shape[-1] < self.exp_size:
